...
session_start();
?>
<?= template_header('Attribution cursus enseignant') ?>
...
if (isset($_REQUEST['btn_update'])) {
    ...
    if (empty($idCursus)) {
        ...
        try {
            if (!isset($errorMsg)) {
                $update_stmt = $db->prepare('UPDATE cursus SET enseignant_id=:uenseignant_id WHERE idCursus=:uidCursus');
                $update_stmt->bindParam(':uenseignant_id', $enseignant_id);
                $update_stmt->bindParam(':uidCursus', $idCursus);
                                        ...
                                        <div class="col-sm-12">
                                            ...
                                            <div class="col-sm-12">
                                                <select name="idCursus">
                                                    ...
                                                    ?>
                                                        <option value="<?php echo $row['idCursus']; ?>"><?php echo $row['matiere']; ?> (<?php echo $row['annee'] ?>)</option>
                                                    <?php
                                                    }
                                                    // $stmt->closeCursor();
                                                    ?>
